...
namespace DestIstok
{
    public class DestIstokDSql : HHandlerDbULoaderIDDest
    {
        /// <summary>
        /// Конструктор - вспомогательный (статическая сборка)
        /// </summary>
        public DestIstokDSql()
            : base()
        {
        }
        ...
        public DestIstokDSql(PlugInULoader iPlugIn)
            : base(iPlugIn)
        {
        }
                    ...
                    && (TableRecieved.Columns.Contains(@"DATETIME") == true))
                {
                    for (int i = 0; i < cntDay; i++)
                    {
                        ...
                        strRes += @"SELECT [ID] as [ID_REC]"
                            ...
                            + @", [ID_TIMEZONE]"
                            + @", [QUALITY]"
                            + @" FROM [" + (_parent as DestIstokDSql).GetNameTable(dtToSelect.GetValueOrDefault()) + @"]"
            ...
            protected override string getTargetValuesQuery()
            {
                ...
                if (m_DupTables.TableDistinct.Rows.Count > 0)
                {
                    ...
                    if (strRows.Equals(string.Empty) == false)
                    {
                        ...
                        strRes = strRes.Replace(@"NAMETABLE_INSERT_INTO", (_parent as DestIstokDSql).GetNameTable(dtToInsert.GetValueOrDefault()));
                        //Лишняя ','
